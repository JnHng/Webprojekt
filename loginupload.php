...
<body>
<form class="loginupload" method="POST" action="login.php?submit=1">
    <b>Login</b><br>
    <br>
...
if(isset($_GET["submit"])) {
    ...
    if (!empty($login) && !empty($passwort)) {
        ...
        $sql = "SELECT * FROM nutzer, files WHERE username='$login' AND passwort='$hash'";
        echo $sql;
        $query = $db->query($sql);
        if ($query == false) {
